...
class videoProcess1(multiprocessing.Process):
    def __init__(self,filename,counter):
        super(videoProcess1, self).__init__()
    ...
    def run(self):
        ...
        with GCSObjectStreamUpload(client=client, bucket_name='tal_interview_app_videos', blob_name=self.filename) as s:
            for x in range(1, int(self.counter)+1):
                with open(self.filename.replace('.mp4','')+str(x)+".txt", "rb") as file:
                    for data in file:
                        datalength = s.write(data)
                os.remove(self.filename.replace('.mp4','')+str(x)+".txt")