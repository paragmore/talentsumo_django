...
if(formid != null && formid.length != 0)
{
	$.ajax({
		...
		{ 
			...
			namefield =  $("#map_name").val();
			emailfield =  $("#map_email").val();
			nomineefield =  $("#map_reviewer").val();
			reviewerfield =  $("#map_nominee").val();
...
function elemclick(e)
{
	...
	if(datalable =="shortText" || datalable =="emailAddress" || datalable =="phoneNumber" || datalable =="passwordField")
	{
		...
		if(datalable == "shortText")
	    {
	        $('#shortTextfields').show();
	        form_field = $('#'+field).attr("data-column");
	        if(namefield.trim().length !=0 && namefield.trim() == form_field)
	        {
	            $('#chklblinputfield').prop("checked",true);
	        }
	        else
	        {
	            $('#chklblinputfield').prop("checked",false);
	        }
	    }
	    else if(datalable == "emailAddress")
	    {
	        $('#emailTextfields').show();
	        form_field = $('#'+field).attr("data-column");
	        if(emailfield.trim().length !=0 && emailfield.trim() == form_field)
	        {
	            $('#selectfieldtype').val("emailfield");
	        }
	        else if(nomineefield.trim().length !=0  && nomineefield.trim() == form_field)
	        {
	            $('#selectfieldtype').val("nomineefield");
	        }
	        else if(reviewerfield.trim().length !=0  && reviewerfield.trim() == form_field)
	        {
	            $('#selectfieldtype').val("reviewerfield");
	        }
	        else
	        {
	            $('#selectfieldtype').val("select");
	        }
	    }
		setupAria(field);
	}
